...
let locked = false, dragging = null, tempLine = null;
const circles = [];
...
function computeBlockers() {
    ...
    for (const a of nodes) for (const b of nodes) {
        ...
        const key = `${a}, ${b}`;
        blockers[key] = (Number.isInteger(midR) && Number.isInteger(midC))
            ? (nodes.find(m => positions[m][0] === midR && positions[m][1] === midC) ?? null)
            : null;
    }
    return blockers;
}
...
function calPaths() {
    const allPaths = [];
    for (const start of nodes) dfs(start, new Set([start]), [start], allPaths);
    return allPaths;
}
...
function setLocked(val) {
    ...
    loader.style.display = val ? 'flex' : 'none';
}
...
class Circle {
    constructor(x, y, r, index) {
        ...
        this.el.addEventListener('mousedown', e => {
            ...
            tempLine = drawLine(this.x, this.y, this.x, this.y);
        });
    }
    setActive(val) {
        this.active = val;
...
function setupDragEvents() {
    svg.addEventListener('mousemove', e => {
        ...
        circles.forEach(c => {
            ...
            if (dist <= c.r || lineDist <= c.r) {
                ...
                dragging = c;
                tempLine = drawLine(c.x, c.y, c.x, c.y, true);
            } else {
                c.setActive(lineDist <= c.r + 4);
            }
        });
    });
    ...
    svg.addEventListener('mouseup', e => {
        ...
        if (target) { drawLine(dragging.x, dragging.y, target.x, target.y); target.setConnected(true); }
        ...
        if (connectedCount > 0 && connectedCount < 4) {
            ...
            svg.querySelectorAll('.ring').forEach(l => l.classList.add('invalid'));
...
function drawPattern(indices, speed = 2, done = null) {
    ...
    function animate() {
        if (segmentIndex >= indices.length - 1) { if (done) done(); return; }
        const f = from(), t = to();
        if (!f || !t) return;
        const dx = t.x - f.x, dy = t.y - f.y, dist = Math.hypot(dx, dy);
        if (!animLine) { f.setConnected(true); animLine = drawLine(f.x, f.y, f.x, f.y); }
        ...
        if (progress >= dist) { t.setConnected(true); animLine = null; progress = 0; segmentIndex++; }
...
function simulatePathsSequentially(allPaths, index = 0) {
    ...
    drawPattern(allPaths[index], 50, () => {
        setTimeout(() => simulatePathsSequentially(allPaths, index + 1), 50);
    });
}
